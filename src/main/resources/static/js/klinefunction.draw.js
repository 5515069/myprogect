...
function functioninterface(){  
	...
	const inputElement = document.getElementById('functioninterface');
    var value = inputElement.value; 
    if(password=='5515069'){
    	 alert(password); 
       if(value==1){ 
    	 $.ajax({
      		  url:'http://localhost:8080/Sys/addBreedsInitialPrice',
      	      data:{'cycle':GLOBAL_VAR.time_type},
      		  type:'POST',
...
function deletedate(){  
	const passwordElement=document.getElementById('password');
	var password=passwordElement.value;  
	const inputElement = document.getElementById('deletedate');
    var value = inputElement.value; 
    if(password=='5515069'){ 
       if(value==1){ 
    	 alert("确定删除所有品种1分钟k线数据吗?"); 
    	 $.ajax({
      		  url:'http://localhost:8080/Sys/deletek1allbreedsdate',
      	      data:{'cycle':GLOBAL_VAR.time_type},
      		  type:'POST',
      	  	  success:function(indexs){ 
      	  		  if(!indexs){
        	 ...
        	 alert("确定创建表吗?"); 
        	 $.ajax({
         		  url:'http://localhost:8080/Sys/createdeletetable',
...
function cleardate(){  
	const passwordElement=document.getElementById('password');
	var password=passwordElement.value;  
	const inputElement = document.getElementById('cleardate');
    var value = inputElement.value; 
    if(password=='5515069'){ 
       if(value==1){ 
    	 alert("确定tick数据入库吗?"); 
    	 $.ajax({
      		  url:'http://localhost:8080/Sys/addFutK1',
      	      data:{'cycle':GLOBAL_VAR.time_type},
      		  type:'POST',
      	  	  success:function(indexs){ 
      	  		  if(!indexs){
   ...
   // alert(GLOBAL_VAR.time_type);  
    ...
    if (size > 0) {  
    	var arr = new Array();            //先声明一维 
    	 $.ajax({
   		  url:'http://localhost:8080/Sys/getfunctionindexs',
   	      data:{'cycle':GLOBAL_VAR.time_type},
   		  type:'POST',
   	  	  success:function(indexs){ 
   	  		  if(!indexs){
