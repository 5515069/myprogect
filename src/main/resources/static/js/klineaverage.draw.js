...
Chart.prototype.updateDataAndDisplay = function () {
    ...
    if (a == -1) {
        ...
        RequestData()
        const inputElement = document.getElementById('average');
        var average = inputElement.value; 
        localStorage.setItem('average', average); 
        
        location.reload();  
        average2 = localStorage.getItem('average'); // 获取保存的值
        alert(average2);
        inputElement.value = average2; // 将保存的值设置到输入元素上
   ...
   // alert(GLOBAL_VAR.time_type);
    ...
    if (size > 0) { 
    	...
    	var arr = new Array();            //先声明一维 
    	 $.ajax({
   		  url:'http://localhost:8080/Sys/getaverage',
   	      data:{'cycle':GLOBAL_VAR.time_type,'average':average,'openopeninterest':openopeninterest,'closeopeninterest':closeopeninterest},
   		  type:'POST',
