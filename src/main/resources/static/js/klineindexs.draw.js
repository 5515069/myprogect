...
var Shape1Indicator = create_class(Indicator);
Shape1Indicator.prototype.__construct = function () {
	Shape1Indicator.__super.__construct.call(this);
    var f = new ParameterExpr("M1", 2, 1000, 1);
    var e = new ParameterExpr("M2", 2, 1000, 1);
    var d = new ParameterExpr("M3", 2, 1000, 60);
    var c = new ParameterExpr("M4", 2, 1000, 0);
    var b = new ParameterExpr("M5", 2, 1000, 0);
    var a = new ParameterExpr("M6", 2, 1000, 6);
    this.addParameter(f);
    this.addParameter(e);
    this.addParameter(d);
    this.addParameter(c);
    this.addParameter(b);
    this.addParameter(a);
    this.addOutput(new RangeOutputExpr("shape1", new Shape1Expr(new VolumeExpr(), f)));
    this.addOutput(new RangeOutputExpr("shape1", new Shape2Expr(new VolumeExpr(), e))); 
...
Shape1Indicator.prototype.getName = function () {
    return "shape1"
};

var Shape1Expr = create_class(RangeExpr);
Shape1Expr.prototype.__construct = function (d, c) { 
	Shape1Expr.__super.__construct.call(this, d, c)
};
Shape1Expr.prototype.calcResult = function (a, d) {   
	return d;
};
var Shape2Expr = create_class(RangeExpr);
Shape2Expr.prototype.__construct = function (d, c) { 
	Shape2Expr.__super.__construct.call(this, d, c)
};
Shape2Expr.prototype.calcResult = function (a, d) {   
	return d;
};
...
ChartSettings.checkVersion = function () {
    if (ChartSettings._data.ver < 2) {
        ...
        a.period_weight = {};
        a.period_weight.line = 11;
        a.period_weight["1min"] = 10;
        a.period_weight["5min"] = 9;
        a.period_weight["15min"] = 8;
        a.period_weight["30min"] = 7;
        a.period_weight["1hour"] = 6;
        a.period_weight["1day"] = 5;
        a.period_weight["1week"] = 4;
        a.period_weight["1month"] = 3;
        a.period_weight["1season"] = 2;
        a.period_weight["1year"] = 1;
...
function clear_refresh_counter() {
    ...
    refresh_handler = setInterval(refresh_function, 10000)
}
...
function clickkind1(){  
	const inputElement = document.getElementById('kind1');
    var value = inputElement.value; 
    localStorage.setItem('kind1value', value); 
    
    localStorage.setItem('breedsvalue', -1); 
    $("#breeds").val(0);
    RequestData();
    location.reload(); 
}
function clickbreeds(){ 
	const inputElement = document.getElementById('breeds');
    var value = inputElement.value; 
    localStorage.setItem('breedsvalue', value); 
    
    localStorage.setItem('kind1value', -1); 
    $("#kind1").val(0);
    RequestData();
    location.reload(); 
}

function clickindexstype(){  
	const inputElement = document.getElementById('indexstype');
    var value = inputElement.value; 
    localStorage.setItem('indexstype', value);  
    RequestData();
    location.reload(); 
}
...
//改写为js方法，使用者要自己写后台台请求，如ajax
var RequestData = function (showLoading) { 
    //显示加载提示
    $("#chart_loading").addClass("activated");
 
    var kind1values=localStorage.getItem('kind1value'); 
    if(kind1values==-1 || !kind1values){
    	$("#kind1").val(0);
    }else{
    	$("#kind1").val(kind1values);
    }
     
    var breedsvalue=localStorage.getItem('breedsvalue');  
    if(breedsvalue==-1 || !breedsvalue){
    	   $("#breeds").val(0);
    }else{
    	   $("#breeds").val(breedsvalue);
    }
 
    var indexstypevalue=localStorage.getItem('indexstype');  
    if(indexstypevalue==-1 || !indexstypevalue){
    	   $("#indexstype").val(0);
    }else{
    	   $("#indexstype").val(indexstypevalue);
    }    
   ...
   // alert(GLOBAL_VAR.time_type);  
    //历史数据
    if (size > 0) {  
    	var arr = new Array();            //先声明一维 
    	 $.ajax({
   		  url:'http://localhost:8080/Sys/getfutureindexs',
   	      data:{'cycle':GLOBAL_VAR.time_type,'kind1value':kind1values,'breedsvalue':breedsvalue
   	    	,'indexstypevalue':indexstypevalue},
   		  type:'POST',
   	  	  success:function(indexs){ 
   	  		  if(!indexs){
   	  			  alert("返回值为空");
   	  		  }
   	  	      for(var i=0;i<JSON.parse(indexs).length;i++){          //一维长度为5
     	            arr[i]=new Array();           //在声明二维  
     	            arr[i][0]=parseInt(JSON.parse(indexs)[i].tradedate);
     	            arr[i][1]=JSON.parse(indexs)[i].open;
     	            arr[i][2]=JSON.parse(indexs)[i].high;
     	            arr[i][3]=JSON.parse(indexs)[i].low;
     	            arr[i][4]=JSON.parse(indexs)[i].close;
     	            arr[i][5]=JSON.parse(indexs)[i].openinterest;; 
     	       }  
   	  	  }
   	  });
        GLOBAL_VAR.KLineData =  arr;//data.datas.data;
        console.log("历史数据");
    } else {
